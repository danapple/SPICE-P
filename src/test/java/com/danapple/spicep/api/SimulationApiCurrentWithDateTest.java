...
public class SimulationApiCurrentWithDateTest {
    @Mock(lenient = true)
    ...
    public void beforeEach() {
        ...
        when(coinCapPriceService.getPrice(eq(SYMBOL_BTC))).thenReturn(CompletableFuture.completedFuture(BigDecimal.valueOf(3)));
        when(coinCapPriceService.getPrice(eq(SYMBOL_ETH))).thenReturn(CompletableFuture.completedFuture(BigDecimal.valueOf(12)));
    ...
    void computesLosingPercentage() {
        ...
        simulateWalletRequest.setDate(LocalDate.now());

        ResponseEntity<?> responseEntity = simulationApi.simulateWallet(simulateWalletRequest);
        ...
        assertThat(response.best_performance()).isEqualByComparingTo(BigDecimal.valueOf(-25));
    }
    ...
    void computesGainingPercentage() {
        ...
        SimulateWalletResponse response = (SimulateWalletResponse) body;
        assertThat(response.best_asset()).isEqualTo(SYMBOL_BTC);
        assertThat(response.best_performance()).isEqualByComparingTo(BigDecimal.valueOf(200));
    }
    ...
    void computesTotal() {
        ...
        SimulateWalletResponse response = (SimulateWalletResponse) body;
        assertThat(response.total()).isEqualByComparingTo(BigDecimal.valueOf(15));
    }
}
